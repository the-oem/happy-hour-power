...
const { checkAuth,
								happyHourParams,
								socialMediaParams,
								patchSocialMedia		} 	= require('./serverMiddleware');
...
app.route('/api/v1/socialmedia/')
.get((req, res) => {

	db('social_media').select()
	.then(data => res.status(200).json({ data }))
	.catch(error => res.status(500).json({ error }))
})
...
app.route('/api/v1/socialmedia/:type')
.get(socialMediaParams, (req, res) => {
	const newType = req.params.type;

	db('social_media').select(newType, 'id')
	.then(data => res.status(200).json({ data }))
	.catch(error => res.status(500).json({ error }))
})
